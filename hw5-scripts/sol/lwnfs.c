...
static int lfs_create(struct inode *dir, struct dentry *dentry, umode_t mode, bool want_excl);
		...
		struct dentry *parent, const char *name)
{
	...
	d_add(dentry, inode);
        ...
        // increment link count on the parent dir
...
static int lfs_mkdir(struct inode *dir, struct dentry *dentry, umode_t mode)
{
    ...
    printk("lfs_mkdir: dentry->d_name.name: %s, mode: %o\n", name, mode);
    printk("\t dentry->d_parent (parent)\n");
    parent = dentry->d_parent;
    printk("\t   %p %s\n", parent, parent->d_name.name);
    printk("\t dir->i_dentry...d_alias (should be same as above)\n");
    hlist_for_each_entry(ptr, &dir->i_dentry, d_alias) { 
        printk("\t   %p\n", ptr);
    } 
    printk("\t parent->d_subdirs...d_u.d_child\n");
    list_for_each_entry(ptr, &parent->d_subdirs, d_u.d_child) { 
        printk("\t   %p %s\n", ptr, ptr->d_name.name);
    } 
    printk("\t dir %s parent->d_inode\n",
            (dir == parent->d_inode) ? "==" : "!=");

    // create an inode

    sb = dir->i_sb;
    inode = lfs_make_inode(sb, S_IFDIR | mode);
    if (!inode) {
        return -ENOMEM;
    }
    inode->i_op  = &lfs_inode_ops;
    ...
    // attach the inode to the given dentry
    d_instantiate(dentry, inode);

    // increment the dentry use count so it won't go away
    dget(dentry);
...
static int lfs_create(struct inode *dir, struct dentry *dentry, umode_t mode, bool want_excl)
{
    ...
    printk("lfs_create: dentry->d_name.name: %s, mode: %o\n", name, mode);
    printk("\t dentry->d_parent (parent)\n");
    parent = dentry->d_parent;
    printk("\t   %p %s\n", parent, parent->d_name.name);
    printk("\t dir->i_dentry...d_alias (should be same as above)\n");
    ...
    sb = dir->i_sb;
    inode = lfs_make_inode(sb, S_IFREG | mode);
    if (!inode)
        return -ENOMEM;
    inode->i_fop = &lfs_file_ops;

    // allocate a counter and attach it to the inode
    // TODO: deallocate on unlink
